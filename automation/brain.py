...
- 長期待機の禁止: 実行時期が先のタスク(P4-P5)しか持たないメンバーには、その本番に
  向けた先行準備・調査タスクを自動創出して割り当ててよい(待機は1日累計4時間まで。
  超過分はオーケストレータが「ロール別バックログ」から稼働保証タスクを采配する)
- 目先のタスクが無い場合でも全員を無理に稼働させる必要はない。割り当てるものが
  無ければ assignments は空でよい(稼働保証は別レイヤーが担う)"""
