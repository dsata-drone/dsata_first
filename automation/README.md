...
3. **待機理由の算出**(compute_waiting): 手空きメンバー全員の「次の一歩の条件」
   (〇〇のタスク完了待ち/補助金採択待ち/社長指示待ち等)を `office_state.json` の
   `waiting` に書き出し、画面の状態欄に「待機中(〇〇待ち)」と表示させる
4. outbox の送信処理
5. ブロッカー・送信失敗・Claudeの判断保留があればエスカレーション
6. `office_state.json` を更新してコミット → オフィス画面が動く
